...
public class TTK91RealAnalyser {
	...
	for (int i = 0; i < screenOutCrit.length; ++i) {
	    System.err.println("Ollaan tarkistamassa outputkriteeriä numero "+i+"/"+screenOutCrit.length);
	    ...
	    if (crit.getQuality()) {
		System.err.println("Tarkastellaan siis laadullista kriteeriä (output)");
		if (!outputQualityOk) {
		    continue; // valmiiksi poks
		}
		qualityFound = true;
		...
		System.err.println("Tarkastellaan siis oikeellisuuskriteeriä (output)");
		if (studPubScreenOut != null) {
		    System.err.print("studPubScreenOutin sisältö: ");
		...
		if (tchrPubScreenOut != null) {
		    System.err.print("tchrPubScreenOutin sisältö: ");
		...
		if ( 
			...
			System.err.print("studHidScreenOutin sisältö: ");
			...
			System.err.print("tchrHidScreenOutin sisältö: ");
		...
		if (!outputCritOk) {
		    System.err.println("Kaik'on'mänt'");
		    break; // kaik' on mänt'
		}
	    }
	}
	...
	// Tuleeko niitä edes? 
	// Rekisterit voisi olla kiva näkyvissä. Samoin ainakin crt-outputit...
	// int[] registers
	// int[] memory
	// int[] crt
	// int[] file
